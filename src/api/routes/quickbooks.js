...
router.get('/auth', async (req, res) => {
  ...
  try {
    ...
    const authUri = QuickBooks.getAuthorizeUrl({
      client_id: process.env.QB_CLIENT_ID,
      scope: 'com.intuit.quickbooks.accounting',
      redirect_uri: process.env.QB_REDIRECT_URI || 'http://localhost:3001/auth/quickbooks/callback',
      response_type: 'code',
      access_type: 'offline'
    });

    console.log('🔗 Generated QuickBooks Authorization URL:', authUri);
...
router.get('/callback', async (req, res) => {
  try {
    ...
    // Store tokens
    userTokens.accessToken = tokenResponse.accessToken;
    userTokens.refreshToken = tokenResponse.refreshToken;
    userTokens.realmId = realmId;
