...
const CashForecastWidget: React.FC<CashForecastWidgetProps> = ({ transactions }) => {
  ...
  // Defensive: Always check scenario exists and data is array
  const scenarioData =
    Array.isArray(forecastWithScenarios) && scenario
      ? forecastWithScenarios.map(w => Number(w[scenario]))
      : [];

  const min = scenarioData.length > 0 ? Math.min(...scenarioData) : NaN;
  const max = scenarioData.length > 0 ? Math.max(...scenarioData) : NaN;

  // Only allow chart if ALL of these are true:
  const canRenderChart =
    scenarioData.length > 0 &&
    scenarioData.every(v => isFinite(v)) &&
    isFinite(min) &&
    isFinite(max) &&
    min !== max && // Don't render for flat line
    (max > 0 || min < 0); // At least one value is nonzero


  console.log('[CashForecastWidget] prediction:', prediction);
  console.log('[CashForecastWidget] forecastWithScenarios:', forecastWithScenarios);
  console.log('[CashForecastWidget] scenario:', scenario);
  console.log('[CashForecastWidget] finalBalance:', finalBalance);
  console.log('[CashForecastWidget] projectedChange:', projectedChange);
  console.log('[CashForecastWidget] projectedChangePercent:', projectedChangePercent);
  console.log('[CashForecastWidget] currentBalance:', currentBalance);
  console.log('[DEBUG CashForecastWidget] scenarioData:', scenarioData);
  console.log('[DEBUG CashForecastWidget] min:', min, 'max:', max, 'canRenderChart:', canRenderChart);
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6">
      ...
      <div className="mb-6">
        ...
        <ResponsiveContainer width="100%" height={300}>
          <AreaChart data={forecastWithScenarios}>
            ...
            <XAxis dataKey="week" />
            <YAxis tickFormatter={(value) => `$${(value / 1000).toFixed(0)}k`} domain={
                min === max
                  ? [min - 1, max + 1]
                  : max <= 0
                  ? [min, 0]
          ...
          <div className="flex items-center justify-center h-[200px] bg-gray-50 rounded-lg">
            <p className="text-gray-500">
              No valid forecast chart data for this period.
            </p>
          </div>
        )}
      </div>
      ...
      <div>
        ...
        {prediction && Array.isArray(prediction?.weeklyForecasts) && prediction.weeklyForecasts.length > 0 && prediction.weeklyForecasts.some(w => w.inflows > 0 || w.outflows > 0) ? (
